...
class Coeff(object):
    def __init__(self,maxX,K0,alpha,gamma,Kappa,Kf,Kb,dA,dB,dY,dZ,concB):
        self.n = 0
        self.nT = 0
        self.xi = 0.0
        self.maxX = maxX
    ...
    def cal_time_steps(self,maxT,deltaT,GammaT):
        T = 0
        while T < maxT:
            T += deltaT
    ...
    def calc_jacob(self,x,deltaT,Theta):
        ...
        for row in range(4,self.n*4-4,4):
            ...
            self.J[row,row] = self.bA[i] + self.Kf*deltaT
            self.J[row,row+1] = -self.Kb*deltaT * x[i+3]
            self.J[row,row+3] = -self.Kb*deltaT*x[i+1]
            self.J[row,row+4] = self.cA[i]

            self.J[row+1,row-3] = self.aB[i]
            self.J[row+1,row] = -self.Kf*deltaT
            self.J[row+1,row+1] = self.bB[i] + self.Kb*deltaT*x[i+3]
            self.J[row+1,row+3] = self.Kb*deltaT*x[i+1]
            ...
            self.J[row+3,row] = -self.Kf*deltaT
            self.J[row+3,row+1] = self.Kb*deltaT*x[i+3]
            self.J[row+3,row+3] = self.bZ[i] + self.Kb*deltaT*x[i+1]
    ...
    def calc_fx(self,x,deltaT,Theta):
        h = self.XX[1] - self.XX[0]
        self.fx[0] = x[4] - x[0]
        ...
        for j in range(4,4*self.n-4,4):
            ...
            self.fx[j] = self.aA[i]*x[4*i-4] + self.bA[i]*x[4*i]+ self.cA[i]*x[4*i+4] + self.Kf*deltaT*x[4*i] -self.Kb*deltaT*x[4*i+1]*x[4*i+3] - self.d[4*i]
            self.fx[j+1] = self.aB[i]*x[4*i-3] + self.bB[i]*x[4*i+1] + self.cB[i]*x[4*i+5] - self.Kf*deltaT*x[4*i] + self.Kb*deltaT*x[4*i+1]*x[4*i+3] - self.d[4*i+1]
            self.fx[j+2] = self.aY[i]*x[4*i-2] + self.bY[i]*x[4*i+2] + self.cY[i]*x[4*i+6] - self.d[4*i+2]
            self.fx[j+3] = self.aZ[i]*x[4*i-1] + self.bZ[i]*x[4*i+3] + self.cZ[i]*x[4*i+7] - self.Kf*deltaT*x[4*i] + self.Kb*deltaT*x[4*i+1]*x[4*i+3] - self.d[4*i+3]
